...
export const meta = {
  ...
  title: "Graphql Stitching - Part 1",
  summary: "What Graphql schema stitching is and how it can help us",
};
...
const typeDefs = gql` type Query { songs: [Song] song(songId: ID!): Song } type Song { id: ID title: String }`;
...
and open the ["http://localhost:3000"](songs api).
...
we can see the [http://localhost:3001](playground) and run our test queries.
